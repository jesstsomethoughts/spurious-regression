# Helper functions to see and save progress while running code/models!
import os
import shutil
import torch
import logging
    ...
    if os.path.exists(folders_util[-1]) and not args.resume and not args.pretrained and not args.evaluate:
        if query_yes_no('overwrite previous folder: {} ?'.format(folders_util[-1])):
            ...
            for folder in folders_util:
                folder_copy = f"{folder}_run2"
                if not os.path.exists(folder_copy):
                    print(f"===> Creating folder copy: {folder_copy}")
...
def calibrate_mean_var(matrix, m1, v1, m2, v2, clip_min=0.1, clip_max=10):
    # DEFINE device here since not using cuda
    device = matrix.device
    m1, v1, m2, v2 = m1.to(device), v1.to(device), m2.to(device), v2.to(device)
