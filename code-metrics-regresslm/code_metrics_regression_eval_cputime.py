...
# Variant: evaluates on cpu_time target instead of memory_bytes
...
def parse_metadata(raw):
    """Parse metadata field, handling both JSON strings and Python-style dicts."""
...
def process_dataset(dataset, SPACE, max_items, language):
    ...
    for row in tqdm(dataset, desc=f"Processing {SPACE} till {max_items} items"):
        if row.get("space") == SPACE and "input" in row and "metadata" in row:
            try:
                meta = parse_metadata(row['metadata'])
                target = float(meta[TARGET_METRIC_TYPE]) # Extract cpu_time from metadata JSON
                lang = meta['language'] if SPACE == "CDSS" else None
    ...
    if parse_errors > 0:
        print(f"  Warning: {parse_errors} rows skipped due to errors (missing '{TARGET_METRIC_TYPE}' or bad metadata)", flush=True)
    if len(inputs) == 0:
        print(f"  ERROR: No rows collected for space '{SPACE}'. Check that metadata contains '{TARGET_METRIC_TYPE}'.", flush=True)
...
def main():
    ...
    # Sanity check
    sample = dataset[0]
    print(f"Dataset sample keys: {list(sample.keys())}", flush=True)
    print(f"Sample space='{sample.get('space')}', metric_type='{sample.get('metric_type')}'", flush=True)
    try:
        meta = parse_metadata(sample['metadata'])
        print(f"Sample metadata keys: {list(meta.keys())}", flush=True)
        print(f"Sample cpu_time from metadata: {meta.get('cpu_time', 'NOT FOUND')}", flush=True)
    except Exception as e:
        print(f"Could not parse sample metadata: {e}", flush=True)

    print(f"Processing {args.max_items} rows of data per space (target: {TARGET_METRIC_TYPE})")
    ...
    for SPACE in args.spaces:
        ...
        if len(inputs) == 0:
            print(f"Skipping {SPACE}: no matching rows found.", flush=True)
            continue
        ...
        if args.verbose:
            print(f"\n  Sample predictions vs targets ({TARGET_METRIC_TYPE}):", flush=True)
        ...
        if args.save:
            ...
            preds_df.to_csv(f"prediction_data_{TARGET_METRIC_TYPE}.csv", index=False)


#### PRINT OUT RESULTS ####
    print(f"\nSummary of Results (target: {TARGET_METRIC_TYPE}):")
    print("="*60)
    print(f"{'Space':<15} {'Spearman ρ':<15} {'MAE':<10}")
    print("-"*60)
    for space in args.spaces:
        if space not in results:
            print(f"{space:<15} {'N/A (no data)'}")
            continue
